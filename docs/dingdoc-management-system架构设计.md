...
`ai/` 采用**功能模块化**模式，每个子功能（knowledge、task\_analysis、tbcreate、terminal、teambition）将 routes 和 service 放在同一目录下，MCP Server 独立进程。AI 代码依赖紧密且不与其他业务共享。
...
- 大同步（每月 1 号 04:00）：对比 remote\_modified\_at 重拉变更文档
...
full\_update 流程：`base/sync/routes.py:full_update()` → 获取分布式锁 → `base/sync/task_sync.py:full_update_service()` → 释放锁
...
- **pgvector (PostgreSQL)**：语义向量存储（chunk\_vectors）— 用于 knowledge-base RAG 检索
- **Perf 库 SQLite**：`performance/` 目录下的季度绩效考核数据
- ORM 使用 SQLAlchemy 2.x，`base/db/engine.py` 管理 scoped\_session（适配 Flask 多线程）
