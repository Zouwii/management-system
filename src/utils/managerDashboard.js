...
export function getAveragePerformanceLabel(rows) {
  if (!rows.length) {
    return '-';
  }

  const averageScore = rows.reduce((sum, row) => sum + (PERFORMANCE_SCORE_MAP[row.perf] ?? 0), 0) / rows.length;

  return PERFORMANCE_LABELS.find((item) => averageScore >= item.min)?.label ?? 'B';
}
...
export function buildTeamSummary(rows) {
  ...
  return {
    ...
    totalHours,
    avgEffectiveRate,
    avgPerformance: getAveragePerformanceLabel(rows),
    mediumRiskCount,
    positiveTrendCount,
    topMember,
  };
