"""AI Teambition task creation service.

Converts AI-generated task drafts into DingTalk / Teambition task tickets.
Uses a default task template for the Ontology Development Department
and maps draft fields to DingTalk API custom fields.
"""
...
# Default template from the Ontology Development Department "软件开发" task sample.
# Individual fields can be overridden via the payload template dict.
...
def _default_due_iso() -> str:
    """Return a default due date (2 days from now) as ISO 8601 string."""
    return (
...
def _normalize_datetime_iso(value: Any, fallback: str) -> str:
    """Normalize a datetime value to UTC ISO 8601 string, using fallback on failure."""
    text = _clean_str(value)
    if not text:
        return fallback
    if text.endswith("Z"):
...
def _build_note(draft: Dict[str, Any], payload: Dict[str, Any]) -> str:
    """Build the note field from draft fields: note, requirementDesc, outputs."""
    ...
    outputs = [
        str(x or "").strip()
        for x in _to_list(draft.get("outputs") or payload.get("outputs"))
        if str(x or "").strip()
    ]
    parts: List[str] = []
    if note:
...
def _build_task_nature_customfield(draft: Dict[str, Any], payload: Dict[str, Any]) -> Dict[str, Any]:
    """Build the DingTalk task-nature customField dict from workType."""
    work_type = _first_text(
        payload.get("workType"), draft.get("workType"),
        payload.get("taskNature"), draft.get("taskNature"),
    )
...
def _build_customfields(draft: Dict[str, Any], payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Build the customfields array for the DingTalk create-task request."""
    raw = (
        payload.get("customfields")
        or payload.get("customFields")
        or draft.get("customfields")
        or draft.get("customFields")
    )
...
def _extract_created_task_id(data: Any) -> str:
    """Extract the created task ID from a DingTalk API response dict."""
...
def build_ai_mission_create_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    """Convert an AI task draft into a DingTalk create-task API payload.

    Args:
        payload: Dict with optional keys: draft, template, executorId, userId,
                 involveMembers, dueDate, startDate, tagIds, etc.
                 The 'draft' sub-dict should contain title, taskType, workType,
                 requirementDesc, outputs, participationLevel.

    Returns:
        A dict ready to POST to the DingTalk create-task endpoint.

    Raises:
        ValueError: If title/content or executorId is missing.
    """
    payload = payload or {}
    ...
    content = _first_text(
        draft.get("title"), draft.get("content"), payload.get("content"), payload.get("title"),
    )
    if not content:
    ...
    start_date = _normalize_datetime_iso(
        payload.get("startDate") or draft.get("startDate"), _utc_now_iso(),
    )
    create_time = _normalize_datetime_iso(
        payload.get("createTime") or draft.get("createTime"), _utc_now_iso(),
    )
    ...
    for output_key, aliases in template_field_map.items():
        ...
        for alias in aliases:
            value = _first_text(
                payload.get(alias), draft.get(alias), template.get(alias), template.get(output_key),
            )
            if value:
                break
        if value:
...
def ai_create_mission_service(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        payload: Must include userId (for the API path) and the draft fields.
                 The request body is built by build_ai_mission_create_payload().

    Returns:
        Dict with success, error, data (taskId, taskUrl, requestPayload), and meta.
    """
    payload = payload or {}
    user_id = _first_text(
        payload.get("userId"), payload.get("userid"),
        payload.get("creatorUserId"), payload.get("executorId"),
    )
    if not user_id:
    ...
    try:
        ...
        return {
            "success": ok_flag,
            "error": "" if ok_flag else "dingtalk create task failed",
            "data": {
