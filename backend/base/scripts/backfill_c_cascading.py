#!/usr/bin/env python3
"""回填 project_task_overdue_details 表的级联三列（project_category_1 / vehicle_type_2 / project_name_3）。"""
...
from base.db.orm import ProjectTaskOverdueDetail
...
def main():
    ...
    try:
        rows = (
            ...
            .filter(ProjectTaskOverdueDetail.raw_json != None)  # noqa: E711
            .filter(ProjectTaskOverdueDetail.raw_json != "")
            .filter(ProjectTaskOverdueDetail.raw_json != "null")
            .filter(
                (ProjectTaskOverdueDetail.project_category_1 == None)  # noqa: E711
                | (ProjectTaskOverdueDetail.vehicle_type_2 == None)  # noqa: E711
                | (ProjectTaskOverdueDetail.project_name_3 == None)  # noqa: E711
            )
            .all()
        )

        print(f"[C 表] 待回填行数: {len(rows)}")
        ...
        session.commit()
        print(f"[C 表] 完成: 回填 {updated} 行, 跳过(无级联数据) {skipped} 行")
