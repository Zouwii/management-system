"""AI task context preparation service.

This module only reads task-related database data and writes markdown context
files for the AI workspace. It never mutates task rows.
"""
...
def load_user_task_context(auth_user: Dict[str, Any], limit: int = 80) -> Dict[str, Any]:
    """Read the current user's task rows and build a read-only context snapshot."""
...
def _render_task_lines(task: Dict[str, Any], task_index: Dict[str, Dict[str, Any]], indent: str = "") -> List[str]:
    lines = []
    ...
    return lines
def _render_task_tree(task: Dict[str, Any], children_by_parent: Dict[str, List[Dict[str, Any]]], task_index: Dict[str, Dict[str, Any]], depth: int = 0) -> List[str]:
    indent = "  " * depth
...
def write_user_task_context_markdown(auth_user: Dict[str, Any], workspace_dir: Optional[Path] = None) -> Dict[str, Any]:
