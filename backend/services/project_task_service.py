...
def search_project_tasks_service(payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    项目任务搜索业务（钉钉）：
    POST /v1.0/project/users/{userId}/tasks/search
    """
    payload = payload or {}
    ...
    params = {
        "roleTypes": payload.get("roleTypes", "creator"),
        "tql": payload.get("tql", "(text ~ '阅读')"),
        "maxResults": payload.get("maxResults", 10),
    }
    if payload.get("nextToken"):
        params["nextToken"] = payload.get("nextToken")
